...
export default function UserSummary() {
  ...
  const loadAssignments = async () => {
    try {
      const [assignments, charges, playersData] = await Promise.all([
      ...
      console.log('💰 UserSummary loaded with overlays:', {
      ...
      calculatePlayerTotals(validAssignments, charges, playersData);
    } catch (error) {
      console.error('Error loading assignments:', error);
    }
  };

  const calculatePlayerTotals = (assignmentsToCalculate: Assignment[], chargesToCalculate: any[] = [], playersData: any[] = []) => {
    ...
    // Process charges (only unpaid charges count towards totals)
    chargesToCalculate.forEach((charge) => {
      const playerName = charge.playerName;
      
      // Skip charges with empty/invalid player names or already paid charges
      if (!playerName || playerName.trim() === '' || charge.status === 'paid') {
        return;
      }
      ...
      // Only process positive charges (debts), negative amounts are payments already processed
      if (validAmount <= 0) {
        return;
      }
      ...
      if (!playerMap[playerName].charges) {
        playerMap[playerName].charges = [];
      }
      
      playerMap[playerName].total += validAmount;
      playerMap[playerName].charges.push(charge);
    });
  ...
  useFocusEffect(
    useCallback(() => {
      console.log('💰 UserSummary screen focused - refreshing assignment data');
      loadAssignments();
    }, [])
  );
  ...
  const renderPlayerItem = ({ item }: { item: PlayerTotal }) => (
    <View style={[styles.playerCard, isDarkMode && styles.darkPlayerCard]}>
      ...
      <View style={styles.actionIcons}>
        ...
        {/* Stripe Payment Icon (if enabled and organization allows it) */}
              ...
              {selectedPlayer?.charges && selectedPlayer.charges.length > 0 && (
                <>
                  ...
                  {selectedPlayer.charges.map((charge, index) => {
                    ...
                    const date = charge.date ? formatUKDateTime(charge.date) : (charge.createdAt ? formatUKDateTime(charge.createdAt) : 'No Date');
                    ...
                    return (
                      <View key={charge.id || `charge-${index}`} style={[styles.breakdownItem, isDarkMode && styles.darkBreakdownItem]}>
                        <View style={styles.breakdownInfo}>
                          ...
                          {charge.description && (
                            <Text style={[styles.chargeDescription, isDarkMode && styles.darkChargeDescription]}>
                              {charge.description}
                            </Text>
                          )}
                        </View>
               ...
               (!selectedPlayer?.charges || selectedPlayer.charges.length === 0) && (
                <View style={styles.emptyBreakdown}>
                  ...
                  <Text style={[styles.emptyBreakdownText, isDarkMode && styles.darkEmptyBreakdownText]}>
                    No items or charges found for this player
                  </Text>
                </View>
              )}
            </ScrollView>
